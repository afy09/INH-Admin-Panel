...
const TambahLembaga = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/datakerjasama/lembaga/create`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <>
      <div className="border rounded-2xl px-7 py-7">
        <Link href="/dashboard/kerja-sama/lembaga">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3 w-full">
            <div className="w-full">
              <label className="block mb-2 text-black-2 font-medium">Nama Lembaga</label>
              <input type="text" className="bg-gray-100 outline-none px-4 py-3.5 w-full text-black-2 placeholder:text-[#DEE4EE] rounded-lg" placeholder="Masukkan Nama Lembaga" value={name} onChange={(e) => setname(e.target.value)} />
            </div>
