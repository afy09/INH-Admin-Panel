...
const TableKategori = ({ dataKategori }: { dataKategori: any }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/datakategori/delete?id=${selectedDeleteId}`, {
        method: "DELETE",
      });
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/datakategori/update?id=${selectedEditData.id}`, {
        method: "PUT",
        headers: {
    ...
    <>
      <div className="border rounded-t-2xl">
        <div className="flex justify-between mt-3">
          <h1 className="text-black-2 px-4 py-3 font-semibold text-xl">Kategori</h1>
        </div>
        <div className="w-full">
          {dataKategori && dataKategori.length > 0 ? (
            <table className="w-full">
              <thead className="bg-amber-50 text-[#252525] font-light text-[14px] rounded-lg">
                <tr>
                  <th className="font-normal p-3">No</th>
                  <th className="font-normal p-3">Nama Kategori</th>
              ...
              <tbody>
                {dataKategori.map((member: any, index: number) => (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black-2 bg-opacity-70 z-999999">
          <div className="bg-white p-8 rounded-2xl shadow-lg ms-90 me-22">
            ...
            <h1 className="text-black-2 font-medium text-xl text-center mt-3">
              Apakah anda yakin <br /> ingin menghapus Kategori ini ?
            </h1>
      ...
      {isPopupOpenEdit && (
        <div className="fixed inset-0 flex items-center justify-center bg-black-2 bg-opacity-70 z-999999">
          <div className="bg-white p-8 rounded-2xl shadow-lg ms-90 me-22">
            ...
            <h1 className="text-black-2 font-medium text-xl text-center mt-3">Edit Kategori</h1>
