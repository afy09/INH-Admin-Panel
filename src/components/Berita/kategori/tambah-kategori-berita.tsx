...
const TambahKategoriBerita = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/datakategori/create`, {
        method: "POST",
        headers: {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3 w-full">
            <div className="w-full">
              <label className="block mb-2 text-black-2 font-medium">Nama Kategori</label>
              <input type="text" className="bg-gray-100 outline-none px-4 py-3 w-full text-black-2 placeholder:text-[#DEE4EE] rounded-lg" placeholder="Masukkan Nama Kategori" value={nama} onChange={(e) => setnama(e.target.value)} />
            </div>
          </div>
...
export default TambahKategoriBerita;
